...
<body>
    <?php
        if (isset($_POST["Insert"])) 
        {
            ...
            $passwordAfterChange = $_POST["nameAfterChange"];
            $userAccount =  $_SESSION['account'];
            $sql = "UPDATE users SET mail_password='".$passwordAfterChange."' WHERE account='".$userAccount."'";
            mysqli_query($conn, 'SET NAMES utf8');
            if (empty($passwordAfterChange))
            {
                header("Location: changeGmailPassword.php?error=名稱不得為空");
                exit();
            }
            else
            {
    ...
    <div class="changeUserName">
        <form action="changeGmailPassword.php" method="post">
            <div class="col-md-6">
                <label>更改APP密碼:</label><br>
