...
<body>
    ...
    echo "<table class='black-table' style='width: 75%; margin: 0 auto;' border='1' id='removeblack'>";
    echo "<colgroup>";
    ...
    echo "    <th style='font-size: 20px; border: 1px solid black;'>系級</th>";
    echo "    <th style='font-size: 20px; border: 1px solid black;'>解除</th></tr>";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td style='border: 1px solid black;' id='removeaccount'>" . $row["account"] . "</td>"; // 假設欄位為 email，根據你的資料庫欄位調整
            echo "<td style='border: 1px solid black;'>" . $row["grade"] . "</td>"; // 假設欄位為 other_info，根據你的資料庫欄位調整
            echo "<td style='border: 1px solid black;'>";
            echo " <input type='button' value='解除' onclick= removeblack(this)>";
            echo "</td>"; 
            echo "</tr>";
        }
        echo "</table>";
    ...
    ?>
    <?php
    $sql = "SELECT * FROM users WHERE blacklist = 0 AND permission = 0 ";
    $result = $conn->query($sql);
    echo "<h2 style= 'text-align: center;'>名單一覽</h2>";
    echo "<table class='black-table' style='width: 75%; margin: 0 auto;' border='1' id='addblack'>";
    echo "<colgroup>";
    echo "<col style='width: 20%;'>"; // 三個欄位均分 75% 的寬度
    echo "<col style='width: 60%;'>";
    echo "<col style='width: 5%;'>";
    ...
    echo "    <th style='font-size: 20px; border: 1px solid black;'>Email</th>";
    echo "    <th style='font-size: 20px; border: 1px solid black;'>系級</th>";
    echo "    <th style='font-size: 20px; border: 1px solid black;'>新增至黑名單</th></tr>";
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td style='border: 1px solid black;'>" . $row["uname"] . "</td>"; // 假設欄位為 username，根據你的資料庫欄位調整
            echo "<td style='border: 1px solid black;'id='addaccount'>" . $row["account"] . "</td>"; // 假設欄位為 email，根據你的資料庫欄位調整
            echo "<td style='border: 1px solid black;'>" . $row["grade"] . "</td>"; // 假設欄位為 other_info，根據你的資料庫欄位調整
            echo "<td style='border: 1px solid black;'>";
            echo " <input type='button' value='新增' onclick= addblack(this) ' style='width:40%; '>";
